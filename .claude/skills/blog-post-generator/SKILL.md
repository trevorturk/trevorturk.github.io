...
- **Say it the way you would say it across a desk.** If a phrase would sound odd spoken to a colleague ("byte-identical", "the whole contract", "provenance is what extraction buys"), say the plain thing: "the same bytes", "the rule is", "the slice is trustworthy because it's copied straight from the source".
- **Contractions and "we" are fine.** A person explaining their own work says "we", "our server", "can't", "it's". Third-person distance ("the team's own server", "the owner rejected those checks as guarding against") is one of the tells. The anonymization rules still apply: no names, and "the owner" or "a reviewer" for people.
- **Subject first, reason after "because".** "The slice is trustworthy because it came from a reviewed PR", not "extracting from the source is what makes the slice trustworthy". No cleft constructions: "X is what makes Y Z", "it is X that Y", "what X does is Y".
- **Cut the sentence that restates the one before it.** "Both were reopened with only the hoists. That made them new code rather than copies." The second sentence explains what the first already said. Keep the one with the fact.
- **The category or the examples, not both.** "Defensive checks, clamping ranges and guarding strides" gives both. "Range checks" is enough.
...
> Some slices can't be a straight copy. Two of the eleven were closed the same day they opened. On top of the hoists, they added range checks on values from our own server, and the owner rejected that as defending against a problem we control. Both were reopened with only the hoists. Those two are new code, not copies, and their PR descriptions say so. A copied slice is trustworthy because it came out of a reviewed PR. A new one has to be reviewed again.

The first plain draft of that paragraph was still not there. It read "Not every slice can be copied straight out of the source PR... That made them new code rather than copies, and their PR descriptions say so. A copy is trusted because it came from a reviewed PR. New code has to earn that trust again, with the build gates and a fresh review." Same facts, but a restating sentence, third-person distance, and a figurative closer. The reader also corrected one of the skill's own examples: "extracting from the source is what makes the slice trustworthy" should be "The slice is trustworthy because it's directly extracted from the source." Subject first, reason after "because", contraction kept. That correction is the calibration point for the whole register. The measure is how it sounds, not how long it is; a plain paragraph can be longer than the AI one.
...
Generated by Claude Fable 5.1 using the blog-post-generator skill. [Brief note about context, iterations, judgment calls, or human edits if any.]
```

Before 2026-09-03 the prompts lived in a How This Post Was Made section at the end of each post; `bin/move-prompts` moved them to `_prompts/` and set the front-matter fields. Reader feedback prompted the move: the section had grown to almost half the file on rewritten posts, and readers who paste a post into an agent want the post.
...
- The original prompts and process note in `_prompts/<slug>.md`.

**Record the pass.** Append a dated **Rewrite (YYYY-MM-DD):** or **Editing pass (YYYY-MM-DD):** paragraph to `_prompts/<slug>.md` with the prompts verbatim and one sentence on what changed in this post. Set `last_edited` to the pass date and `last_edited_by` to the model, in both the post's and the prompts file's front matter.

**Verify with `bin/verify-post _posts/<file>.md`.** It diffs the fenced blocks and `##` headings against `main` (ignoring `##` lines inside fences), checks the date, internal links, `{% raw %}` balance, that `_prompts/<slug>.md` still carries the original prompt lines and has a Rewrite or Editing pass paragraph dated today (override with `PASS_DATE=YYYY-MM-DD`), that `last_edited` matches, and prints the prose word count, the retired-word hits, and the count of sentences over 25 words, before and after. The register report is informational: a hit on a post whose subject is the word ("adversarial review") is fine; a hit used as a flourish is not. Then `bundle exec jekyll build`. State both in the PR. `--allow-code-changes` and `--allow-link-changes` exist for the fact-check pass and for deliberately retargeted links.
...
5. **Record it.** Append **Fact check (YYYY-MM-DD):** to `_prompts/<slug>.md` with the prompt verbatim and what was corrected, and update `last_edited` and `last_edited_by` in both front matters. Omit the paragraph when nothing changed.
