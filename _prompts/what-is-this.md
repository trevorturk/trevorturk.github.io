...
**Rewrite (2026-09-03):** Plain-register pass, pilot for issue #66, after a reader said the posts read like AI. Archive batch 5, the last, run after batch 4 (#72) merged. This post is the blog's about essay, so the pass was light and its own headings stayed. Fragments became sentences ("It spreads. Through training data. Through RAG systems." is now three plain sentences, with "retrieval system" in place of RAG), "consume and propagate" became "read and pass along", the three stacked metaphors closing the candle section became one sentence with two of them ("genetic transfer" dropped as a repeat of plant cuttings), and "LLM" became "model" throughout. Judgment calls: the closing line "a candle loses nothing by lighting another" stays despite the no-quotable-closer rule, because it's the credited saying the post is built on, not a coined aphorism; "high-friction" left the body but the Low Friction heading is fixed. Prompts, verbatim:
