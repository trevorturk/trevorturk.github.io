...
**Rewrite (2026-09-03):** Plain-register pass, pilot for issue #66, after a reader said the posts read like AI. Archive batch 5, the last, run after batch 4 (#72) merged. This post already read fairly plainly after the September 1 rewrite, so the changes are mostly small: the summary, the opening, and the Why This Works reasons are now said the way a person would say them, cleft constructions in Results and Lessons Learned were turned around ("The report is what you reread" became "You'll reread the report"), "pros and cons, trade-offs" collapsed to one term, and the handoff document is called "implementation prompt" throughout instead of rotating with "handoff document" and "implementation instructions". Judgment calls: the Devise example in the opening now says "installing" instead of "implementing" because that is the word a person would use; the bullet lists under When to Use and The Artifacts were reworded but keep every item; the table, blockquotes, code blocks, headings, links, and the hypothetical-example note from the fact check are unchanged. Prompts, verbatim:
