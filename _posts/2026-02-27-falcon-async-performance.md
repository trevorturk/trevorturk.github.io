...
**Editing pass (2026-09-01):** The owner asked, "did we have a post about our major CPU savings work we did a while back? IIRC we cut our Heroku bill by a huge amount w/ Claude doing local benchmarks and finding hotspots" and then "kick off a post in a PR for that". One sentence was added to the Benchmarking section linking to the new post on that pass; nothing else changed.
